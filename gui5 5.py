...
class RectanglePacker:
    def __init__(self, dx=0, dy=0, packed_canvas_width=297, packed_canvas_height=210):
        self.dx = dx
        self.dy = dy
        self.canvas_width = packed_canvas_width
        self.canvas_height = packed_canvas_height
...
class OrganPipeGenerator:
    ...
    def bbox(self, x, y, xw, yw, label):
        spacing = self.object_spacing / 2
        rect_element = ET.Element('rect', {
            'x': f'{x - spacing}',
            'y': f'{y - spacing}',
            'width': f'{xw + self.object_spacing}',
            'height': f'{yw + self.object_spacing}',
    ...
    def create_stopper_group(self, parent_group, x, y, width, depth, mt, lt, radius):
        ...
        # Binnenste opening: rond of vierkant
        if self.Round_handle:
            if debug: 
                print(f"Round tuning handle active for pipe {self.pipe_number}, handle diameter = {self.Handle_dia} mm")
            radius_circle = self.Handle_dia / 2
            cx = x + outer_width / 2
            cy = y + outer_height / 2
            inner_circle = ET.SubElement(stopper_group, 'circle', {
                'cx': str(cx),
                'cy': str(cy),
                'r': str(radius_circle),
                'style': 'fill:none;stroke:red'
    ...
    def generate_parts_table(self):
        self.parts_table = []

        def add(part, width, height):
            self.parts_table.append({
                "Pipe": self.pipe_number,
                "Label": self.label,
                "Part": part,
                "Width (mm)": round(float(width), 2),
                "Height (mm)": round(float(height), 2)
            })

        for i in range(3):
            add(f'fb c {i}', self.width, self.depth)
        for i in range(3):
            add(f'fb{i}', self.width, self.depth)
        add('fbb', self.width, self.foot_height)
        if not self.No_ears:
            add('lab', self.width, 5 * self.mouth_height)
        else:
            add('lab', self.width + 2 * self.wall_thickness, 5 * self.mouth_height)
        add('frt', self.width, self.length - 2 * self.mouth_height)
        for i in range(5):
            add(f'stp{i}', self.width - 2 * self.leather_thickness, self.depth - 2 * self.leather_thickness)
        if not self.Round_handle:
            for i in range(2):
                add(f'TH {i}', self.wall_thickness * 6, 10 + self.wall_thickness * 5)
        add('S1', self.depth + 2 * self.wall_thickness, self.length + self.foot_height)
        add('S2', self.depth + 2 * self.wall_thickness, self.length + self.foot_height)
        add('Bck', self.width, self.length + self.foot_height)
        if not self.No_ears:
            add('ft_f', self.width + 2 * self.wall_thickness, self.foot_height + 3 * self.mouth_height)
        else:
            add('ft_f', self.width + 2 * self.wall_thickness, self.foot_height)
        for i in range(6):
            add(f'rig{i}', self.width, self.depth)
        for i in range(2):
            add(f'rc{i}', 2 * self.wall_thickness, self.length + self.foot_height)
        if not self.No_ears:
            add('ft_fs', self.width + 2 * self.wall_thickness, self.foot_height + 3 * self.mouth_height)
        else:
            add('ft_fs', self.width + 2 * self.wall_thickness, self.foot_height)
    ...
    def generate_svg(self, canvas_width= 210 , canvas_length = 297, units = "mm",file_path="",pipe_number=1):
       ...
       #foot front seal
        ...
        # --- File writing logic ---
        if not self.fill_canvas:
            if debug:
                print('hier wordt de file gesloten')
            file_name = os.path.join(file_path, f'organ_pipes_{self.label.replace(" ", "_").lower()}.svg')
            self.document = ET.ElementTree(root)

            tree = self.document
            root = tree.getroot()

            #rect_packer = RectanglePacker(packed_canvas_width=canvas_width, packed_canvas_height=canvas_length)
...
class OrganPipeGUI:
    def __init__(self, master):
        ...
        self.default_values = {
            'midi_numbers_comma_separated': '53,58,60,62,63,64,65,67,69,70,72,74,75,76,77,79,81,82,84,86',
        ...
        checkbox = tk.Checkbutton(
            master,
            text="Round tuning handle:",
            variable=var,
            command=self.toggle_handle_dia_entry
        )
        ...
        self.toggle_handle_dia_entry()

        row_counter += 1

        # Define the part checkbox visibility toggle BEFORE using it
        def toggle_part_checkboxes():
            if self.checkboxes["make_parts_table_only"].get():
                self.parts_checkbox_frame.grid()
            else:
                self.parts_checkbox_frame.grid_remove()

        self.toggle_part_checkboxes = toggle_part_checkboxes

        # List of parts
        self.available_parts = [
            "fb0", "fb1", "fb2", "fb c 0", "fb c 1", "fb c 2",
            "fbb", "lab", "frt", "stp0", "stp1", "stp2", "stp3", "stp4",
            "TH 0", "TH 1", "S1", "S2", "Bck", "ft_f", "ft_fs",
            "rig0", "rig1", "rig2", "rig3", "rig4", "rig5", "rc0", "rc1"
        ]

        # "Make Parts Table Only" Checkbox
        self.checkboxes["make_parts_table_only"] = tk.BooleanVar()
        self.checkboxes["make_parts_table_only"].set(False)
        self.parts_table_checkbox = tk.Checkbutton(
            master,
            text="Make Parts Table Only",
            variable=self.checkboxes["make_parts_table_only"],
            command=self.toggle_part_checkboxes
        )
        self.parts_table_checkbox.grid(row=row_counter, column=0, sticky='w')
        row_counter += 1

        # Part checkbox frame (initially hidden)
        self.part_checkboxes = {}
        self.part_checkbox_vars = {}
        self.parts_checkbox_frame = tk.LabelFrame(master, text="Parts to include in table")
        self.parts_checkbox_frame.grid(row=row_counter, column=0, columnspan=5, sticky='w')
        row_counter += 1

        for i, part_name in enumerate(self.available_parts):
            var = tk.BooleanVar(value=True)
            cb = tk.Checkbutton(self.parts_checkbox_frame, text=part_name, variable=var)
            cb.grid(row=i // 6, column=i % 6, sticky='w')
            self.part_checkboxes[part_name] = cb
            self.part_checkbox_vars[part_name] = var
    ...
    def create_organ_pipe(self):
        ...
        parameters['handle_dia_mm'] = float(self.handle_dia_entry.get()) if parameters['round_tuning_handle'] else 0
        parameters['make_parts_table_only'] = parameters.get('make_parts_table_only', False)

        included_parts = [
            part for part, var in self.part_checkbox_vars.items() if var.get()
        ]

        shared_document_generator = None
        pipe_rows = []

        for i, midi_number in enumerate(parameters['midi_numbers'], start=1):
            ...
            opg = OrganPipeGenerator(
                width=organ_pipe.width_rectangular,
                length=organ_pipe.pipe_length,
                foot_height=parameters['foot_height'],
                wall_thickness=parameters['wall_thickness'],
                leather_thickness=parameters['leather_thickness'],
                depth=organ_pipe.depth_rectangular,
                mouth_height=organ_pipe.mouth_height,
                label=f'm{organ_pipe.midi_number}',
                frequency=int(organ_pipe.frequency),
            ...
            if parameters['make_parts_table_only']:
                opg.generate_parts_table()
                row = {'Pipe': i, 'Label': opg.label}
                for part in opg.parts_table:
                    name = part['Part']
                    if name in included_parts:
                        row[f"{name}_w"] = part['Width (mm)']
                        row[f"{name}_h"] = part['Height (mm)']
                pipe_rows.append(row)
            else:
                if parameters['fill_canvas']:
                    if shared_document_generator is None:
                        shared_document_generator = opg
                    else:
                        opg.document = shared_document_generator.document
            ...
            self.message_field.insert(tk.END, f"Pipe {i} processed.\n")

        if parameters['make_parts_table_only'] and pipe_rows:
            import csv
            csv_path = os.path.join(parameters['directory_path'], 'pipe_parts_wxh.csv')

            all_keys = set()
            for row in pipe_rows:
                all_keys.update(row.keys())

            ordered_keys = ['Pipe', 'Label']
            part_names = sorted(set(k[:-2] for k in all_keys if k.endswith('_w')))
            for name in part_names:
                ordered_keys.append(f"{name}_w")
                ordered_keys.append(f"{name}_h")
            ...
            self.message_field.insert(tk.END, f"Saved pipe parts table to: {csv_path}\n")

        elif parameters['fill_canvas'] and shared_document_generator is not None:
            root = shared_document_generator.document.getroot()
            rect_packer = RectanglePacker(
                packed_canvas_width=parameters['document_width'],
                packed_canvas_height=parameters['document_height']
            )
            packed_roots = rect_packer.pack_rectangles(root)
            rect_packer.save_svg_files(packed_roots, parameters['directory_path'])

            combined_path = os.path.join(parameters['directory_path'], 'organ_pipes_combined.svg')
            with open(combined_path, 'wb') as f:
                shared_document_generator.document.write(f, pretty_print=True)


if __name__ == "__main__":
    import sys

    original_stdout = sys.stdout  # bewaar stdout

    def on_close():
        sys.stdout = original_stdout  # herstel stdout
        root.destroy()

    root = tk.Tk()
    app = OrganPipeGUI(master=root)
    root.protocol("WM_DELETE_WINDOW", on_close)  # sluit veilig af
    root.mainloop()
...
class TextRedirector(object):
    ...
    def write(self, str):
        try:
            ...
            pass  # GUI is al gesloten, geen widget meer beschikbaar
